...
- **Chroma 持久化与缓存校验**：Chroma 负责本地向量数据库持久化，manifest 用于判断源文档、embedding 模型和分块策略变化，避免重复构建或误用过期索引。
...
- `storage/chroma/` 是 Chroma 本地运行产物，删除后会在下次预热或问答时基于知识库重新生成。
- Chroma 负责向量库持久化；`index_manifest.json` 仅用于判断源文档、embedding 模型和分块策略是否变化。
